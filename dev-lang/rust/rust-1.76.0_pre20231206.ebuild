...
if [[ ${PV} = *beta* ]]; then
	...
	nightlyver=${PV//*_pre}
	NIGHTLY_SNAPSHOT="${nightlyver:0:4}-${nightlyver:4:2}-${nightlyver:6:2}"
	MY_P="rustc-nightly"
	SLOT="nightly/${PV}"
	SRC="${NIGHTLY_SNAPSHOT}/rustc-nightly-src.tar.xz -> rustc-${PV}-src.tar.xz"
	INSTALL_COMPLETIONS="no"
	#force_nightly="|| ( miri nightly )"
else
	ABI_VER="$(ver_cut 1-2)"
	SLOT="stable/${ABI_VER}"
	MY_P="rustc-${PV}"
...
PATCHES=(
	...
	"${FILESDIR}"/1.75.0-handle-vendored-sources-when-remapping.patch
)
